...
function getCategoryStyle(category?: PresaleCategory) {
  switch (category) {
    ...
    case "gaming":
      return { bg: "bg-[#FF7F41]", label: "Gaming" };
    case "infrastructure":
      return { bg: "bg-[#B8EF53]", label: "Infra" };
    case "meme":
      return { bg: "bg-[#FFE28A]", label: "Meme" };
    default:
      return { bg: "bg-[#F3EAD8]", label: "Other" };
  }
}
...
export function PresaleCard({ presale }: { presale: PresaleWithStatus }) {
  ...
  const category =
    presale.category || (metadata?.category as PresaleCategory | undefined);
  ...
  return (
    <Link to={`/projects/${presale.address}`}>
      <div className="relative h-full cursor-pointer flex flex-col border-[3px] border-black bg-white p-6 [box-shadow:0_0_0_1px_#000,10px_10px_0_0_#000] transition-all duration-200 group hover:[box-shadow:0_0_0_1px_#000,14px_14px_0_0_#000] hover:-translate-x-1 hover:-translate-y-1">
        {/* Status indicator */}
        <div
          className={`absolute top-0 right-0 w-5 h-5 translate-x-1 -translate-y-1 rotate-[8deg] border-[3px] border-black ${getStatusColor()}`}
        ></div>

        {/* Category badge */}
        <div
          className={`absolute top-4 left-4 -rotate-[3deg] px-3 py-1 ${categoryStyle.bg} border-[3px] border-black text-[10px] font-black uppercase tracking-[0.14em]`}
        >
          {categoryStyle.label}
        </div>
        ...
        <p className="font-bold mb-6 min-h-[3rem] text-black/85">
          {description}
        </p>
        ...
        <div className="mt-auto">
          ...
          {project.statusType === "completed" && (
            <div className="pt-6 border-t-[3px] border-black text-center">
              <p className="font-black uppercase text-sm tracking-[0.14em] mb-2">
                PROJECT COMPLETED
              </p>
              <button className="text-xs font-black uppercase underline hover:no-underline tracking-[0.14em]">
                VIEW RESULTS →
              </button>
            </div>
          )}

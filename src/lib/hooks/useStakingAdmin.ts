...
export function useStakingAdmin() {
  ...
  const { isSuccess: isSetApySuccess, isError: isSetApyError } =
    useWaitForTransactionReceipt({
      hash: setApyTxHash as `0x${string}` | undefined,
    });

  const { isSuccess: isAddToWhitelistSuccess, isError: isAddToWhitelistError } =
    useWaitForTransactionReceipt({
      hash: addToWhitelistTxHash as `0x${string}` | undefined,
    });

  const { isSuccess: isSupplyRewardsSuccess, isError: isSupplyRewardsError } =
    useWaitForTransactionReceipt({
      hash: supplyRewardsTxHash as `0x${string}` | undefined,
    });

  const { isSuccess: isReleaseRewardsSuccess, isError: isReleaseRewardsError } =
    useWaitForTransactionReceipt({
